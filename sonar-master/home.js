...
function initial(){
    ...
    */
    ...
    if(data.status== true){
        ...
        let hotPost= data.hotPost;
        // for start:
        for(var i= 0; i< hotPost.length; i++){
            console.log("aaaaaa");
            var onePost= "<div class=\"single-hero-slide bg-img slide-background-overlay\" style=\"background-image: url(img/bg-img/slide1.jpg);\">"+
            ...
            // onePost+= "<h2>"+ hotPost[i].shopName+ "&nbsp;<strong>"; // 店名
            // onePost+= hotPost[i].foodName+ "</strong></h2>"; // 餐點名
            // onePost+= "<i class=\"fa fa-dollar\" style=\"color:darkgoldenrod\">"+ hotPost[i].foodCost+ "元</i>&nbsp;"; // 金額
            // onePost+= "<i class=\"fa fa-fire\" style=\"color:darkred\">"+ hotPost[i].foodCal+ "cal </i>"; // 熱量
            // onePost+= "<p>"+ hotPost[i].foodNote+ "</p></div></div></div></div></div>";
            
            // $(".owl-carousel").owlCarousel('add', onePost).owlCarousel('update');
            // $("section .hero-area .hero-slide .owl-carousel").append(onePost);
        }
        // for end.
        ...
        for(var i= 0; i< recordPost.length; i++){
            ...
            $("section .container-fluid .sonar-portfolio").append(onePost);
        }
        // for end.
    }
    else{
